  ...
  const columns = [
    {
      ...
      render: (row: Category) => (
        <div className="flex items-center gap-3">
          {row.image?.url ? (
            <img src={row.image.url} alt={row.name} className="h-9 w-9 rounded-lg object-cover" />
          ) : (
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-muted text-lg">
              {row.icon || <Tags className="h-4 w-4 text-muted-foreground" />}
            </div>
          )}
    ...
    },
    {
      key: 'slug',
      header: 'Slug',
      render: (row: Category) => <span className="text-muted-foreground">{row.slug}</span>,
    },
    {
      key: 'productCount',
      header: 'Products',
      render: (row: Category) => <span>{row.productCount ?? 0}</span>,
    },
    {
      key: 'isActive',
      header: 'Status',
      render: (row: Category) => <StatusIndicator status={row.isActive ? 'active' : 'inactive'} />,
    },
    {
      key: 'actions',
      header: '',
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <FormField label="Image">
                  <div className="flex items-center gap-3">
                    {form.image?.url && (
                      <img src={form.image.url} alt="preview" className="h-12 w-12 rounded-lg object-cover" />
                    )}
