...
export default function BrandProductsPage() {
  ...
  return (
    <div className="container py-6 lg:py-8">
      <Breadcrumb
        items={[{ label: 'Brands', href: '/brands' }, { label: title }]}
        className="mb-6"
      />
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-center gap-4">
          {brand?.logo?.url && (
            <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-xl border border-border bg-card p-2">
          ...
          <div>
            ...
            {brand?.description ? (
              <p className="mt-1 max-w-2xl text-body-md text-muted-foreground">{brand.description}</p>
            ) : (
              <p className="mt-1 text-body-md text-muted-foreground">
                {pagination ? `${pagination.totalResults} products` : 'Browse products from this brand'}
              </p>
            )}
          </div>
        </div>
