...
export async function generateManualInvoicePdf(outputPath) {
  ...
  if (fs.existsSync(logoPath)) {
    try {
      doc.image(logoPath, 36, headerY, { width: 115 });
    } catch (e) {
      // Fallback
    }
  }
  ...
  doc.strokeColor('#e2e8f0').lineWidth(1).moveTo(36, divY1).lineTo(559, divY1).stroke();
  ...
  doc.font(fontBold).fontSize(9.5).fillColor('#111827').text('EngineersBuy Instruments', 46, partyY + 22);
  ...
  doc.font(fontBold).fontSize(9.5).fillColor('#111827').text('IEEE STB MMMUT', 317, partyY + 22);
  doc.font(fontRegular).fontSize(8.5).fillColor('#475569')
  ...
  let rowY = summaryY + 8;
  const renderSummaryRow = (label, value, isBold = false, isAccent = false) => {
